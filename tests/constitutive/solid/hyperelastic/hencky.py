from conspire.constitutive.solid.hyperelastic import Hencky
import numpy as np
...
model = Hencky(bulk_modulus, shear_modulus)


def test_str():
    assert (
        model.__str__()
        == "Hencky(" + f"bulk_modulus={bulk_modulus}, shear_modulus={shear_modulus})"
    )
